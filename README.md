...
4. **Phân định minh bạch trách nhiệm 3 tầng phía bệnh viện:**
   * **Bác sĩ (`ACT-002`):** Cấu hình Master Template chuẩn hóa và xử lý ngoại lệ lâm sàng.
   * **Điều dưỡng (`ACT-003`):** Kích hoạt hồ sơ bệnh nhân trong <30s và in phiếu QR tại quầy lưu viện.
...
Toàn bộ hệ thống tài liệu trong kho lưu trữ đã được đồng bộ 100% theo tài liệu chuẩn mực trung tâm **[US_US_V1.md](file:///d:/EXE101/DOC_UC_DB_PROJECT_EXE101/US_US_V1.md)**:
...
| **06** | **Phân Tích Mô Hình Dữ Liệu** | [`01_Database_Analysis.md`](file:///d:/EXE101/DOC_UC_DB_PROJECT_EXE101/01_Database_Analysis.md) | Phân tích 20 thực thể dữ liệu nghiệp vụ, thuộc tính, khóa chính/ngoại, chỉ mục và ma trận thao tác CRUD đối ứng với các ca sử dụng chuẩn. |
| **07** | **Sơ Đồ Thực Thể Quan Hệ (ERD)** | [`02_ERD.md`](file:///d:/EXE101/DOC_UC_DB_PROJECT_EXE101/02_ERD.md) | Sơ đồ Master ERD Mermaid tổng thể hệ thống và 3 sơ đồ phân rã phân hệ dữ liệu, bảng tổng hợp khóa ngoại và hành vi On Delete. |
| **08** | **Thiết Kế Cơ Sở Dữ Liệu Vật Lý** | [`03_Database_Design.md`](file:///d:/EXE101/DOC_UC_DB_PROJECT_EXE101/03_Database_Design.md) | Đặc tả vật lý chi tiết 24 bảng CSDL PostgreSQL (kiểu dữ liệu, ràng buộc, default, indexes tối ưu và phân vùng dữ liệu theo 5 chi nhánh). |

---

## 3. KIẾN TRÚC TRUY VẾT YÊU CẦU (REQUIREMENT TRACEABILITY ARCHITECTURE)
...
[Functional Requirements: F-001 .. F-028]
    ...
    [User Stories: US-001 .. US-035] ──► file_user_story.md
                   │
                   ▼
    [Use Cases: UC-001 .. UC-028]    ──► file_use_case_spec.md
        │                    │
        ▼                    ▼
[UI: SCR-CG/DOC]     [Entities & Tables]
04_Screen_Flow.md    01_Database_Analysis.md / 02_ERD.md / 03_Database_Design.md
        │                    │
        └──────────┬─────────┘
                   ▼
